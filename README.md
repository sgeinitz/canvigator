...
per-question histograms are only generated by `get-quiz-submission-events`.
...
**Before running:** the `get-quiz-submission-events` task must have been run first for the same
...
| | `data/<course>/<quiz>_<id>_all_submissions_YYYYMMDD.csv` (from `get-quiz-submission-events`) |
...
**Typical workflow:**
1. Run `python canvigator.py get-quiz-submission-events` to export submission data for all quizzes.
...
#### `get-quiz-submission-events` — Export quiz submissions and events
